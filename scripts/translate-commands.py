...
TRANSLATIONS = {
    ...
    # 斜杠命令描述（Discord/聊天界面）- 全部
    '"Show available commands."': '"显示可用命令。"',
    '"List all slash commands."': '"列出所有斜杠命令。"',
    '"Run a skill by name."': '"按名称运行技能。"',
    '"Show current status."': '"显示当前状态。"',
    '"List/add/remove allowlist entries."': '"列出/添加/移除白名单条目。"',
    '"Approve or deny exec requests."': '"批准或拒绝执行请求。"',
    '"Explain how context is built and used."': '"解释上下文的构建和使用方式。"',
    '"Export current session to HTML file with full system prompt."': '"导出当前会话为 HTML 文件（含完整系统提示）。"',
    '"Control text-to-speech (TTS)."': '"控制文本转语音（TTS）。"',
    '"Show your sender id."': '"显示你的发送者 ID。"',
    '"Manage session-level settings (for example /session idle)."': '"管理会话级设置（如 /session idle）。"',
    '"List, kill, log, spawn, or steer subagent runs for this session."': '"列出、终止、查看日志、生成或引导此会话的子代理运行。"',
    '"Manage ACP sessions and runtime options."': '"管理 ACP 会话和运行时选项。"',
    '"Bind this thread (Discord) or topic/conversation (Telegram) to a session target."': '"将此线程（Discord）或主题/会话（Telegram）绑定到会话目标。"',
    '"Kill a running subagent (or all)."': '"终止运行中的子代理（或全部）。"',
    '"Usage footer or cost summary."': '"用量摘要或成本统计。"',
    '"Stop the current run."': '"停止当前运行。"',
    '"Restart OpenClaw."': '"重启 OpenClaw。"',
    '"Reset the current session."': '"重置当前会话。"',
    '"Compact the session context."': '"压缩会话上下文。"',
    '"Set thinking level."': '"设置思考级别。"',
    '"Toggle verbose mode."': '"切换详细模式。"',
    '"Toggle reasoning visibility."': '"切换推理可见性。"',
    '"Set exec defaults for this session."': '"设置此会话的执行默认值。"',
    '"List model providers or provider models."': '"列出模型提供商或提供商模型。"',
    '"Adjust queue settings."': '"调整队列设置。"',
    '"Switch to ${dock.id} for replies."': '"切换到 ${dock.id} 进行回复。"',
